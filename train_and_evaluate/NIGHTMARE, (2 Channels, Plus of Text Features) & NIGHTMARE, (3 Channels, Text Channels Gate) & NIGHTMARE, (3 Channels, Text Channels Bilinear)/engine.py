...
def train_one_epoch(
    ...
    if args.modality == 'text':
        ...
        for sequence1, sen1, sequence2, sen2, sequence3, sen3, candidates, y, sents in tqdm(dataloader):
            ...
            s1, sequence1 = channel(s1, s1b, sequence1)
            sequence1 = torch.cat((sequence1, s1), dim=2)
            ...
            s3, sequence3 = channel(s3, s3b, sequence3)
            sequence3 = torch.cat((sequence3, s3), dim=2)
            ...
            can_sens, candidates = channel(can_sens, can_sensb, candidates)
            candidates = torch.cat((candidates, can_sens), dim=2)
            ...
            sen_emb, sequence = channel(sen_emb, sen_embb, sequence)
            sequence = torch.cat((sequence, sen_emb), dim=2)
