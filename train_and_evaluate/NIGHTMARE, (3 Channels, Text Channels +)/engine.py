...
def train_one_epoch(
    model, model_head, dataloader, device, epoch, optimizer, logger, args, combiner, channel, constraint
):
    ...
    if args.modality == 'text':
        ...
        for sequence1, sen1, sequence2, sen2, sequence3, sen3, candidates, y, sents in tqdm(dataloader):
            ...
            sen1, sen1b = torch.split(torch.tensor(np.array(sen1)), 768, dim=2)
            sen2, sen2b = torch.split(torch.tensor(np.array(sen2)), 768, dim=2)
            sen3, sen3b = torch.split(torch.tensor(np.array(sen3)), 768, dim=2)
            sents, sentsb = torch.split(torch.tensor(np.array(sents)), 768, dim=2)
            ...
            s1b = reducer(torch.tensor(np.array(sen1b), dtype=torch.float32))
            s1b = s1b.permute(1, 0, 2)
            s1b = s1b.to(device)

            s1, s1b, sequence1 = channel(s1, s1b, sequence1)
            sequence1 = torch.cat((sequence1, s1 + s1b), dim=2)
            ...
            s3b = reducer(torch.tensor(np.array(sen3b), dtype=torch.float32))
            s3b = s3b.permute(1, 0, 2)
            s3b = s3b.to(device)


            s3, s3b, sequence3 = channel(s3, s3b, sequence3)
            sequence3 = torch.cat((sequence3, s3 + s3b), dim=2)
            ...
            can_sensb = reducer(torch.tensor(np.array(sentsb), dtype=torch.float32))
            can_sensb = can_sensb.permute(1, 0, 2)
            can_sensb = can_sensb.to(device)
            can_sens, can_sens_b, candidates = channel(can_sens, can_sensb, candidates)
            candidates = torch.cat((candidates, can_sens + can_sens_b), dim=2)

            
            sen_emb = reducer(torch.tensor(np.array(sen2), dtype=torch.float32))
            sen_emb = sen_emb.permute(1, 0, 2)
            sen_emb = sen_emb.to(device)
            sen_embb = reducer(torch.tensor(np.array(sen2b), dtype=torch.float32))
            sen_embb = sen_embb.permute(1, 0, 2)
            sen_embb = sen_embb.to(device)
            

            sen_emb, sen_emb_b, sequence = channel(sen_emb, sen_embb, sequence)
            sequence = torch.cat((sequence, sen_emb + sen_emb_b), dim=2)
...
def eval_one_epoch(
    model, model_head, dataloader, device, epoch, logger, best_acc, args, combiner, channel, constraint
):
    ...
    if args.modality == 'text':
        ...
        for _, _, sequence, sent, _, _, candidates, y, (s_comic, s_panel), (c_comics, c_panels), sents in tqdm(dataloader):
        
            b, s, f = sequence.shape
            

            sent, sentb = torch.split(torch.tensor(np.array(sent)), 768, dim=2)
            sents, sentsb = torch.split(torch.tensor(np.array(sents)), 768, dim=2)
            ...
            with torch.no_grad():
                ...
                sen_emb = sen_emb.permute(1, 0, 2)
                sen_emb = sen_emb.to(device)
                sen_embb = reducer(torch.tensor(np.array(sentb), dtype=torch.float32))
                sen_embb = sen_embb.permute(1, 0, 2)
                sen_embb = sen_embb.to(device)
