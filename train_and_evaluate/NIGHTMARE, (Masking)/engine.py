...
def train_one_epoch(
    ...
    if args.modality == 'text':
        ...
        for sequence1, sen1, sequence2, sen2, sequence3, sen3, candidates, y, sents in tqdm(dataloader):
            ...
            #condition_gate = torch.sigmoid(constraint).view(1, 1, -1)
            #condition_gate = condition_gate.to(device)
            ...
            #can_sens_zero = torch.zeros_like(can_sens)
            #can_sens_zero = can_sens_zero.to(device)
            # Blend the real and zeroed versions
            mask1 = torch.bernoulli(torch.full_like(can_sens, 0.5))
            can_sens = mask1 * can_sens
            #can_sens = condition_gate * can_sens + (1 - condition_gate) * can_sens_zero
            ...
            #sen_emb_zero = torch.zeros_like(sen_emb)
            #sen_emb_zero = sen_emb_zero.to(device)
            # Blend the real and zeroed versions
            #sen_emb = condition_gate * sen_emb + (1 - condition_gate) * sen_emb_zero
            mask2 = torch.bernoulli(torch.full_like(sen_emb, 0.5))
            sen_emb = mask2 * sen_emb
