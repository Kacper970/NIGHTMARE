...
# =============================================================================
# def reduce(sent, model_name='bert-base-uncased', target_dim=384):
# 
# 
...
# # 
# # =============================================================================
#     # Initialize the dimensionality reducer with input size 768 (BERT's output size) and output size target_dim
#     reducer = DimensionalityReducer(768, 384)
# 
#     # Tokenize the input sentence and convert it to input IDs
#     final = []
#     
#     for sen in sentence:
#         
#         
#         inputs = tokenizer_text(sen, return_tensors="pt", padding=True, truncation=True, max_length=512)
#     
#         
...
#     
#         # Get the last hidden state (the embeddings for the sentence)
#         last_hidden_state = outputs.last_hidden_state
#     
#         # To get a fixed-length feature vector, we can pool the last hidden state.
#         # One common approach is to take the mean of all token embeddings.
#         sentence_embedding = last_hidden_state.mean(dim=1).squeeze()
#     
#         # Alternatively, you can take the embedding of the [CLS] token as a sentence representation
#         #cls_embedding = last_hidden_state[:, 0, :].squeeze()
#     
#         # Reduce dimensionality from 768 to the target dimension
#         reduced_embedding = reducer(sentence_embedding)
#         
#         final.append(reduced_embedding)
#         
# 
...
def train_one_epoch(
    model, model_head, dataloader, device, epoch, optimizer, logger, args, combiner, channel
):
...
def eval_one_epoch(
    model, model_head, dataloader, device, epoch, logger, best_acc, args, combiner, channel
):
